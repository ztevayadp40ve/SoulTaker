...
class MakeEnv:
    ...
    def get_src(self) -> None:
        ...
        """
        subprocess.run(
            ['git', 'clone', 'https://github.com/MrShadowDev/SoulTaker.git'], cwd=self.build_dir, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        shutil.move(os.path.join(self.build_dir,
                    'SoulTaker', 'src'), self.build_dir)
